...
from public_user_credit_purchases_testdata import data as td
...
class PublicUserPurchaseTransactions(unittest.TestCase):
    ...
    def test_validate_usd_purchase_pkg1(self):
        ...
        try:
            ...
            print "\n Testcase: Public user purchases USD package1"
            print " Amount to purchase: %s\n"%(td.test_pkg1['USD'])
            ...
            print '\n- Verification. Going to Client List'
            self.driver.get("%s"%td.test_url)
            self.cf.login(self.driver, td.test_admin_username, td.test_admin_password)
            client_info = self.cf.navigate_to_clients(self.driver, "%s %s"%(td.user1['name'], td.user1['surname']))
            print "-- Client Name: %s"%(client_info['name'])
            print "-- Email Address: %s"%(client_info['email'])
            print "-- Balance: %s"%(client_info['balance'])
            print "-- Assert balance \n-- Expected: %s \n-- Live: %s" %("%.2f%s"%(float(td.test_pkg1['USD']),"USD"), client_info['balance'])
            self.assertEqual("%.2f%s"%(float(td.test_pkg1['USD']),"USD"), client_info['balance'])
            self.cf.screenshot(self.api_session, self.id, "Verification public user purchase")
            self.driver.get("%s"%td.test_url)
            self.cf.logout(self.driver)
